// -*- C++ -*-
//
// Package:    ThreePointCorrelatorEtaGapTracker
// Class:      ThreePointCorrelatorEtaGapTracker
// 
/**\class ThreePointCorrelatorEtaGapTracker ThreePointCorrelatorEtaGapTracker.cc CMEandCorrelation/ThreePointCorrelatorEtaGapTracker/src/ThreePointCorrelatorEtaGapTracker.cc
...
ThreePointCorrelatorEtaGapTracker::ThreePointCorrelatorEtaGapTracker(const edm::ParameterSet& iConfig)
...
ThreePointCorrelatorEtaGapTracker::~ThreePointCorrelatorEtaGapTracker()
{
...
void
ThreePointCorrelatorEtaGapTracker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
...
// initialize Qcos and Qsin
  ...
  double Qrp[NetaBins][2];//Q vector for 3rd particles, no charge different
  double Qrp_count[NetaBins];
     ...
     math::XYZPoint bestvtx(bestvx,bestvy,bestvz);
        ...
        for(int eta = 0; eta < NetaBins; eta++){
          if( trkEta > etaBins_[eta] && trkEta < etaBins_[eta+1] ){
            ...
            if( trk.charge() == 1 || trk.charge() == -1 ){

              Qrp[eta][0] += weight*cos( -2*trk.phi() );
              Qrp[eta][1] += weight*sin( -2*trk.phi() );

              Qrp_count[eta] += weight;
            }
          }
        }     
  } 
...
//3p correlators
  for(int ieta = 0; ieta < NetaBins; ieta++){
    for(int jeta = 0; jeta < NetaBins; jeta++){
      for(int keta = 0; keta < NetaBins; keta++){
    
      double deltaEta12 = fabs(etaBins_[ieta] - etaBins_[jeta]);
        ...
        if( deltaEta12 > dEtaBinsArray[deta] && deltaEta12 < dEtaBinsArray[deta+1] ){
          if( deta == 0 ){
            for(int sign = 0; sign < 2; sign++){
              ...
              delEta3p[sign]->Fill( deltaEta12 );
              double Q_real = get3RealOverlap(Q1[ieta][sign][0], Q2[ieta][sign][0], Qrp[keta][0], Q1[ieta][sign][1], Q2[ieta][sign][1], Qrp[keta][1], Q1_count[ieta][sign], Q2_count[ieta][sign], Qrp_count[keta] );
              QvsdEta[deta][sign]->Fill( Q_real, (Q1_count[ieta][sign]*Q1_count[ieta][sign] - Q2_count[ieta][sign])*Qrp_count[keta] );
            }

            if( Q1_count[ieta][0] == 0 || Q1_count[ieta][1] == 0 || Qrp_count[keta] == 0.0 ) continue;

            delEta3p[2]->Fill( deltaEta12 );
            double Q_real = get3Real(Q1[ieta][0][0]/Q1_count[ieta][0],Q1[jeta][1][0]/Q1_count[jeta][1],Qrp[keta][0]/Qrp_count[keta], Q1[ieta][0][1]/Q1_count[ieta][0], Q1[jeta][1][1]/Q1_count[jeta][1], Qrp[keta][1]/Qrp_count[keta]);
            QvsdEta[deta][2]->Fill( Q_real, Q1_count[ieta][0]*Q1_count[jeta][1]*Qrp_count[keta] );  

          }
          else{
              for(int sign = 0; sign < 2; sign++ ){
              
                if( Q1_count[ieta][sign] == 0.0 || Q1_count[jeta][sign] == 0.0 || Qrp_count[keta] == 0.0 ) continue; 

                delEta3p[sign]->Fill( deltaEta12 );
                double Q_real = get3Real(Q1[ieta][sign][0]/Q1_count[ieta][sign],Q1[jeta][sign][0]/Q1_count[jeta][sign],Qrp[keta][0]/Qrp_count[keta], Q1[ieta][sign][1]/Q1_count[ieta][sign], Q1[jeta][sign][1]/Q1_count[jeta][sign], Qrp[keta][1]/Qrp_count[keta]);
                QvsdEta[deta][sign]->Fill( Q_real, Q1_count[ieta][sign]*Q1_count[jeta][sign]*Qrp_count[keta] );  

              }

              if( Q1_count[ieta][0] == 0.0 || Q1_count[jeta][1] == 0.0 || Qrp_count[keta] == 0.0 ) continue;
...
void 
ThreePointCorrelatorEtaGapTracker::beginJob()
{
...
//HF:
  ...
  for(int deta = 0; deta < NdEtaBins; deta++){
    for(int sign = 0; sign < 3; sign++){
      ...
      QvsdEta[deta][sign] = fs->make<TH1D>(Form("QvsdEta_%d_%d", deta, sign), "", 20000, -1.0-0.00005, 1.0-0.00005);
    }
  }
...
double 
ThreePointCorrelatorEtaGapTracker::get3Real(double R1, double R2, double R3, double I1, double I2, double I3) 
{
...
double ThreePointCorrelatorEtaGapTracker::get3RealOverlap(double R1, double R2, double R3, double I1, double I2, double I3, double N1, double N2, double N3){
...
double ThreePointCorrelatorEtaGapTracker::get3Imag(double R1, double R2, double R3, double I1, double I2, double I3){
...
double ThreePointCorrelatorEtaGapTracker::get3ImagOverlap(double R1, double R2, double R3, double I1, double I2, double I3, double N1, double N2, double N3){
...
double ThreePointCorrelatorEtaGapTracker::get2Real( double R1, double R2, double I1, double I2){

      double real = R1*R2 - I1*I2;
      return real;

}
double ThreePointCorrelatorEtaGapTracker::get2RealOverlap( double R1, double R2, double I1, double I2){

      double real = R1*R1 - I1*I1 - R2;
      return real;

}
double ThreePointCorrelatorEtaGapTracker::get2Imag( double R1, double R2, double I1, double I2){

      double imag = R1*I2 + R2*I1;
      return imag;
}
double ThreePointCorrelatorEtaGapTracker::get2ImagOverlap( double R1, double R2, double I1, double I2){
...
void 
ThreePointCorrelatorEtaGapTracker::endJob() 
{
}

// ------------ method called when starting to processes a run  ------------
void 
ThreePointCorrelatorEtaGapTracker::beginRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a run  ------------
void 
ThreePointCorrelatorEtaGapTracker::endRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when starting to processes a luminosity block  ------------
void 
ThreePointCorrelatorEtaGapTracker::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void 
ThreePointCorrelatorEtaGapTracker::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
ThreePointCorrelatorEtaGapTracker::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(ThreePointCorrelatorEtaGapTracker);